# =============================================================================
# Simulation of 6-compartment chicken GIT model (0-48h, control)
# =============================================================================
# Runs BacArena-based FBA simulation of a 6-compartment chicken
# gastrointestinal tract model (Gizzard, Duodenum, Jejunum, Ileum,
# Cecum, Colon) for 48 timesteps (hours), modeling day/night cycles,
# peristalsis, reverse peristalsis (reflux), nutrient absorption,
# and inter-compartment transfer of bacteria and metabolites.
#
# USAGE:   Rscript Simulation_coupling_6comps_args_universal_growerdiet_add24hMets.R
#            <econc_proportion> <prop_gizzard> <sample> <mucus_TF> <timesteps>
#            <Sugars_X> <AminoAcid_X>
#
# INPUTS:  Diet tables, OTU table, metadata, parameter tables (day/night),
#          metabolite concentrations at h24, exchange constraints, model lists
# =============================================================================
...
cl <- makeCluster(cores, type = "PSOCK",
                  outfile = file.path(cluster_log_dir,
                                      paste0("cluster_coupling_6comps_10d_night_GAPSEQ_48h_",
                                             ...
                                             "_propGizzard_", prop_gizzard, "_mucusUrea_",
                                             today,
...
# =============================================================================
# Main parallel simulation loop (one iteration per replicate)
# =============================================================================
print(system.time(simlist <- parLapply(cl, 1:replicates, function(i) {

  # --- Set seed for this replicate -------------------------------------------
  # (used in CreateModelArena function, 239 + seedN)
  N <- sample(c(1:100), 1)
  seedN <- as.integer(239 + N)
  print(paste0("SEED FOR THIS REPLICATE IS ", as.character(seedN)))

  # ===========================================================================
  # Helper functions (defined inside parLapply for cluster worker access)
  # ===========================================================================

  # --- Reverse peristalsis (reflux): move fraction from comp N+1 to comp N ---
  ...
  # --- Set oxygen concentration in arena -------------------------------------
  ...
  # --- Add metabolites from hour 24 concentrations ---------------------------
  # Mets were collected at time point 24 when the proportion of the medium added was X90,
  # that is why conc_multiply = prop_gizzard/90, i.e. scaling the added concentrations
  # (instead of essential concentrations) to the medium added
  addMets_from24h <- function(arena, sampleName, compartment, conc_multiply = 1) {
    ...
    Metabolite_conc_h24_sub <- Metabolite_conc_h24[which(
      Metabolite_conc_h24$Compartment == compartment &
  ...
  # --- Save simulation outputs for a compartment at a timestep ---------------
  save_sim_outputs <- function(sim_object, out_lists, comp_idx, timestep) {
    ...
    # Save only negative shadow prices (metabolite limitation indicators)
  ...
  # --- Update type-specie table when new species are transferred -------------
  ...
  # --- Remove organisms by name from arena -----------------------------------
  ...
  # --- Split ileum removal results between cecum and colon -------------------
  split_removal_to_cecum_colon <- function(removefunction_result, fraction_to_cecum, fraction_to_colon) {
    ...
    # Update species transfer tables
  ...
  # ===========================================================================
  # Handle Sugars_X / AminoAcid_X "None" -> NULL conversion
  # ===========================================================================
  if (Sugars_X == "None") {
    Sugars_X <- NULL
  }
  ...
  # --- Update exchange reaction constraints for each compartment model list --
  ...
  # --- Organisms to remove from specific compartments ------------------------
  ...
  # ===========================================================================
  # Initialize arenas for all 6 compartments
  # ===========================================================================
  ...
  essential_mets_gizzard <- CreateModelArena(
    modelList = model_lists$Gizzard, organisms = organism_lists[[1]], sampleName = sample,
    day = "D10", compartment = "Gizzard", diet_name = diet_name,
    proportion = prop_gizzard, initial_commsize = Parameters_table$initial_community_size[1],
  ...
  for (timestep in 1:timesteps) {
    ...
    if (timestep %in% c(1:16, 25:40, 49:64)) {
      ...
      if (timestep %% as.numeric(Parameters_table$diet_added_hrs[1]) == 0) {
        arena2_gizzard <- AddBacDiet_gizzard(
          Chicken_organisms_gizzard = organism_lists[[1]],
      ...
      if (!(timestep %% reflux_duo_frequency == 0)) {
        ...
        removefunction_result_gizzard <- RemoveBacMetabolites(
          sim_gizzard, arena2_object = arena2_gizzard,
      ...
      # although once every N hrs reflux happens --> reverse peristalsis duodenum->gizzard
      # --> nothing being transferred to jejunum and nothing getting in duodenum from gizzard:
      reflux_duo <- FALSE
      if (!(timestep %% reflux_duo_frequency == 0)) {
        # NO REFLUX
        ...
        removefunction_result_duodenum <- RemoveBacMetabolites(
          sim_duodenum, arena2_object = arena2_duodenum,
        ...
        arena2_duodenum <- AddBacMets_fromUpstreamCompartment(
          removefunction_result_n0 = removefunction_result_gizzard,
        ...
        compartment_arenas[[1]] <- Reflux(
          sim_object_n1 = sim_duodenum, perc_to_remove_bac_n1 = perc_to_reflux_duodenum,
      ...
      removefunction_result_jejunum <- RemoveBacMetabolites(
        sim_jejunum, arena2_object = arena2_jejunum,
      ...
      # although once every N hrs reflux happens --> reverse peristalsis duodenum->gizzard
      # --> nothing being transferred to jejunum; if no reflux - add bacs and mets from duodenum:
      if (reflux_duo == FALSE) {
        arena2_jejunum <- AddBacMets_fromUpstreamCompartment(
          removefunction_result_n0 = removefunction_result_duodenum,
      ...
      removefunction_result_ileum <- RemoveBacMetabolites(
        sim_ileum, arena2_object = arena2_ileum,
      ...
      # Split ileum removal between cecum and colon
      ileum_split <- split_removal_to_cecum_colon(removefunction_result_ileum, fraction_to_cecum, fraction_to_colon)
      removefunction_result_ileum_to_cecum <- ileum_split$to_cecum
      removefunction_result_ileum_to_colon <- ileum_split$to_colon

      # add bacs and mets from jejunum:
      arena2_ileum <- AddBacMets_fromUpstreamCompartment(
        removefunction_result_n0 = removefunction_result_jejunum,
      ...
      if (timestep %% 4 == 0) {
        ...
        removefunction_result_cecum <- RemoveBacMetabolites(
          sim_cecum,
      ...
      # add metabolites and bacteria removed from the ileum (fraction of ileal contents)
      arena2_cecum <- AddBacMets_fromUpstreamCompartment(
        removefunction_result_n0 = removefunction_result_ileum_to_cecum,
      ...
      removefunction_result_colon <- RemoveBacMetabolites(
        sim_colon,
      ...
      if (timestep %% 4 == 0) {
        arena2_colon <- AddBacMets_fromUpstreamCompartment(
          removefunction_result_n0 = removefunction_result_cecum,
      ...
      arena2_colon <- AddBacMets_fromUpstreamCompartment(
        removefunction_result_n0 = removefunction_result_ileum_to_colon,
        sim_object_n1 = sim_colon, arena2_n1 = arena2_colon,
        model_list_n1 = model_lists$Colon,
        model_list_n0 = model_lists$Ileum)
      ...
      # absorb water: leave 20% of water in colon
    ...
    if (timestep %in% c(17:24, 41:48, 65:72)) {
      ...
      # set pH and oxygen for all compartments
      for (i in 1:6) {
        compartment_arenas[[i]] <- set_pH(compartment_arenas[[i]],
                                          Parameters_table$pH[Parameters_table$Compartment == names(compartment_arenas)[i]])
        compartment_arenas[[i]] <- set_o2(compartment_arenas[[i]],
                                          as.numeric(Parameters_table$oxygen_mM[Parameters_table$Compartment == names(compartment_arenas)[i]]))
      }
      ...
      # --> reverse peristalsis duodenum->gizzard:
      if (!(timestep %% reflux_freq_hrs_duo == 1)) {
        ...
        # IF REFLUX - gizzard just gets its arena (no removal downstream)
        arena2_gizzard <- BacArena::getArena(sim_gizzard, 1)
      }
      ...
      if (!(timestep %% as.numeric(Parameters_table_night$reflux_freq_hrs[Parameters_table_night$Compartment == "Jejunum"]) == 1)) {
        ...
        # REFLUX: jejunum transfers fraction upstream to duodenum
        perc_to_reflux_jejunum <- as.numeric(Parameters_table_night$reflux_perc[Parameters_table_night$Compartment == "Jejunum"])
        compartment_arenas[[2]] <- Reflux(
          sim_object_n1 = sim_jejunum, perc_to_remove_bac_n1 = perc_to_reflux_jejunum,
      ...
      # when NO REFLUX - stuff transferred from ileum downstream to cecum and colon;
      ...
      if (!(timestep %% as.numeric(Parameters_table_night$reflux_freq_hrs[Parameters_table_night$Compartment == "Ileum"]) == 1)) {
        # NO REFLUX
        # select % that will be split between cecum and colon
        ...
        compartment_arenas[[3]] <- Reflux(
          sim_object_n1 = sim_ileum, perc_to_remove_bac_n1 = perc_to_reflux_ileum,
      ...
      # IF REFLUX - then nothing is transferred from ileum
      # (fraction of colon will be transferred upstream in next section)
      ...
      ## during the night nothing gets removed from colon (except during reflux)
      ...
      if (!(timestep %% as.numeric(Parameters_table_night$reflux_freq_hrs[Parameters_table_night$Compartment == "Ileum"]) == 1)) {
        ...
        removefunction_result_colon <- RemoveBacMetabolites(
          sim_colon, arena2_object = arena2_colon,
          perc_to_remove_bac = perc_to_remove_colon_bac,
          perc_to_remove_mets = perc_to_remove_colon_mets,
          type_specie_table = type_specie_table_colon)
        arena2_colon <- removefunction_result_colon[[1]]
        ...
        # then just leave remaining fraction for cecum:
        ...
        # change the slot [[4]] - species_rm (table with names of specie models and numbers transferred)
      ...
      # -----------------------------------------------------------------------
      # ILEUM AND CECUM AGAIN: add refluxes from colon (night only)
      # -----------------------------------------------------------------------
      ...
      if (timestep %% as.numeric(Parameters_table_night$reflux_freq_hrs[Parameters_table_night$Compartment == "Colon"]) == 1) {
        # add metabolites and bacteria removed from the colon to ileum
        compartment_arenas[[4]] <- AddBacMets_fromDownstreamCompartment(
          removefunction_result_colon_to_ileum,
          sim_object_n0 = sim_ileum,
          arena2_n0 = compartment_arenas[[4]],
          model_list_n0 = model_lists$Ileum, model_list_n1 = model_lists$Colon)

        # imitate absorption of sugars, amino acids and fats; and conversion of starches in ileum:
        compartment_arenas[[4]] <- AbsorptionConversion_ileum(arena2 = compartment_arenas[[4]], diet)

        # add metabolites and bacteria removed from the colon to cecum
        compartment_arenas[[5]] <- AddBacMets_fromDownstreamCompartment(
          removefunction_result_colon_to_cecum,
          sim_object_n0 = sim_cecum,
          arena2_n0 = compartment_arenas[[5]],
          model_list_n0 = model_lists$Ceca, model_list_n1 = model_lists$Colon)
      }
  ...
  # ===========================================================================
  # Assemble output simlist for this replicate
  # ===========================================================================
...
saveRDS(simlist, file.path(output_dir,
                           paste0(sample, "_", as.character(diet_name),
                                  "_diet_grower_NEW_", as.character(timesteps),
