###############################################################################
# CompileTables_fromOUTfiles_Controls.R
#
# Compile extracted simulation outputs (from Extract_OUTfiles scripts)
# into analysis-ready tables for CONTROL simulations. Concatenates
# the 0-48h and 48-96h simulation halves, extracts SCFA fluxes and
# concentrations, and writes summary tables for downstream analyses
# (PCoA, fold change plots, etc.).
#
# USAGE (command line):
#   Rscript CompileTables_fromOUTfiles_Controls.R \
...
base_out_dir   <- "path/to/simulation_outputs/CoupledSixComps_D10/NoPathogens/2024/gapseq_models/OUT_files/"
...
for (file in relab_tables_48h) {
  ...
  relab_table   <- meltTheRelabTable(out_tables[[2]], control = TRUE)
...
for (file in relab_tables_48_96h) {
  ...
  absab_table_melt <- reshape2::melt(absab_table,
    id.vars = c("Specie", "Replicate", "Sample", "medium",
                ...
                "Sugars_X", "AminoAcids_X"))
  colnames(absab_table_melt)[c(10, 11)] <- c("Hour", "AbsAbundance")
  ...
  relab_table <- meltTheRelabTable(relab_table, control = TRUE)

  # Shift hours: 1-48 -> 49-96 (continuation of first 48h)
...
write.table(relab_table_grower_h96,
  paste0(path_to_tables, "relab_table_", pattern_arg_48_96h, "_h96_control_", sim_date_48_96h, "_meanByRep.txt"),
  sep = '\t', quote = FALSE)
write.table(absab_table_grower_h96,
  paste0(path_to_tables, "absab_table_", pattern_arg_48_96h, "_h96_control_", sim_date_48_96h, "_meanByRep.txt"),
  sep = '\t', quote = FALSE)
write.table(relab_tables_grower_96h,
  paste0(path_to_tables, "RelAbund_tables_96h_", pattern_arg_48_96h, "_", sim_date_48_96h, "_meanByRep.txt"),
  sep = '\t', quote = FALSE)
write.table(commsize_tables_grower_96h,
  paste0(path_to_tables, "CommSize_tables_96h_", pattern_arg_48_96h, "_", sim_date_48_96h, "_meanByRep.txt"),
  sep = '\t', quote = FALSE)
...
#' @param sim_date          date subdirectory name
#' @param out_tables_files  vector of OUTfile filenames
#' @return list of 6 data.tables: SCFA flux sums by community, SCFA flux means
#'         by species, SCFA concentrations, all metabolite concentrations,
#'         total community flux sums, total community flux sums per hour
get_flux_conc_tables <- function(sim_date, out_tables_files) {
  ...
  for (out_table_file in out_tables_files) {
    ...
    for (comp_n in 1:6) {
      ...
      # Community-level flux sums (all metabolites)
      flux_community_eachHour <- flux_table[, .(Flux = sum(Flux)),
        by = .(Reaction, Replicate, Hour, proportion_medium, medium, SampleName, Compartment)]
      flux_community_sum <- flux_table_sum[, .(Flux = sum(Flux)),
        by = .(Reaction, Replicate, proportion_medium, medium, SampleName, Compartment)]

      # SCFA community-level sum
...
flux_scfa_bySpecie_96h <- rbindlist(list(tables_48h[[2]], tables_48_96h[[2]]))[,
  ...
  by = .(Reaction, Specie, proportion_medium, medium, SampleName, Compartment)]
...
write.table(conc_scfa_96h[Hour == 95, ],
  paste0(path_to_tables, "conc_table_hour96_SCFA_control_", pattern_arg_48_96h, "_sim", sim_date_48_96h, ".txt"),
  sep = '\t', quote = FALSE)

write.table(flux_scfa_96h,
  paste0(path_to_tables, "flux_table_sum_SCFA_ByCommunity_control_", pattern_arg_48_96h, "_sim", sim_date_48_96h, ".txt"),
  sep = '\t', quote = FALSE)

write.table(flux_scfa_bySpecie_96h,
  paste0(path_to_tables, "flux_table_sum_SCFA_bySpecie_meanByRep_control_", pattern_arg_48_96h, "_sim", sim_date_48_96h, ".txt"),
  sep = '\t', quote = FALSE)

write.table(conc_scfa_96h,
  paste0(path_to_tables, "conc_table_96h_SCFA_control_", pattern_arg_48_96h, "_sim", sim_date_48_96h, ".txt"),
  sep = '\t', quote = FALSE)

write.table(flux_allMets_96h,
  paste0(path_to_tables, "flux_table_96h_sum_allMets_ByCommunity_control_", pattern_arg_48_96h, "_sim", sim_date_48_96h, ".txt"),
  sep = '\t', quote = FALSE)
